...
export class UnclearedLockFileError extends FloccusError {
  ...
  constructor(lockFile: string) {
    super(
      `E023: Unable to clear lock file, consider deleting ${lockFile} manually.`
    );
    this.code = 23;
...
export class LockFileError extends FloccusError {
  ...
  constructor(status: number, lockFile: string) {
    super(
      `E024: HTTP status ${status} while trying to determine status of lock file ${lockFile}`
    );
    this.code = 24;
    this.status = status;
    this.lockFile = lockFile;
...
export class FailsafeError extends FloccusError {
  ...
  constructor(percent: number) {
    super(
      `E029: Failsafe: The current sync run would delete ${percent}% of your bookmarks. Refusing to execute. Disable this failsafe in the profile settings if you want to proceed anyway.`
    );
    this.code = 29;
...
export class OAuthTokenError extends FloccusError {
  constructor() {
    super(
      "E032: OAuth error. Token validation error. Please reconnect your Google Account."
    );
    this.code = 32;
...
export class RedirectError extends FloccusError {
  constructor() {
    super(
      "E033: Redirect detected. Please make sure the server supports the selected sync method and URL you entered is correct doesn't redirect to a different location."
    );
    this.code = 33;
...
export class FileUnreadableError extends FloccusError {
  constructor() {
    super(
      "E034: Remote bookmarks file is unreadable. Did you forget to set an encryption passphrase?"
    );
    this.code = 34;
...
export class CreateBookmarkError extends FloccusError {
  ...
  constructor(bookmark: Bookmark) {
    super(
      `E035: Failed to create the following bookmark on the server: ${bookmark}`
    );
    this.code = 35;
    this.bookmark = bookmark;
